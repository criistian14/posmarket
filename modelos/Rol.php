...
class Rol
{
    protected static $tablaConsulta = 'roles';
	...
	public static $consultaSelect = "SELECT * FROM roles ";
	 ...
	 * Para acceder a los campos, es de la siguiente manera $rol[0]->reporte
	*/
	 ...
	 * Para acceder a los campos, es de la siguiente manera $rol[0]->rol
	...
	*/
	...
	public static function resultado()
	{
		...
		while ( $rol = $resultado->fetch_assoc() ) {
			...
			// Añadir los campos al tipo de rol
			$rolTemporal->id 	 = ( isset($rol['id']) ? $rol['id'] : '');
			$rolTemporal->rol 	 = ( isset($rol['rol']) ? $rol['rol'] : '');
		...
		static::$consultaSelect = 'SELECT * FROM roles ';
	...
	public function guardar()
	{
		...
		if ($this->update) {
			...
			$sentencia = $conexion->conn->prepare("UPDATE ". static::$tablaConsulta . " SET rol= ? WHERE id= ?");
